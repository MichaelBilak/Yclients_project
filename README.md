...
Этот же импорт обновляет метки услуг из листа `services`. Формат листа меток: `Категория`, `id_услуги`, `Название услуги`, `доп услуга`. Также поддерживаются старые алиасы `service_id`, `company_id`, `branch`, `service_title`, `is_extra`, `tag`. Значения `да`, `доп`, `extra`, `1` считаются доп. услугой; `нет`, `0`, `обычная` снимают метку. Метка хранится в разрезе филиала: если услуга есть только в одном филиале, она попадает в общий расчет сети и в расчет этого филиала, но не влияет на остальные филиалы. Worker автоматически сверяет этот лист раз в 7 дней (`SERVICES_LABEL_SYNC_INTERVAL_DAYS=7`; `0` отключает автосверку). Результат последней сверки хранится в `system.sync_state` с ключами `services_labels_*`.
